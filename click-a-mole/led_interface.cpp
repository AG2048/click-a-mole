...
{ // vector bc for one round it may be 3 lives or 5 lives so theats like 3 vs 5 colors, its green by default but if they do pass smth then itll be what they pass
...
void DisplayInterface::change_mole_hp(int mole_id, int new_hp, int max_hp)
{
    ...
    for (int i = animation_list.size() - 1; i >= 0; i--)
    {
        ...
        if (animation->mole_id == mole_id && animation->animation_type == AnimationCategory::Solid && animation->led_type == LedType::Ring && animation->animation_to_return_to != nullptr)
        {
            ...
            { // cap flash at end time and remove animation to return to
...
void DisplayInterface::process_timed_animations(unsigned long current_time_ms)
{
    ...
    for (int i = animation_list.size() - 1; i >= 0; i--)
    {
        ...
        if (animation->end_time_ms <= current_time_ms)

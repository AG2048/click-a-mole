...
void Mole::handleDeath(GameLogic &game)
{
    ...
    HP = game.getInitialHP();                                           // set mole's HP to initial HP
    setPosition(false, game.getDisplayInterface());       // set mole position to down
    game.setScore(game.getScore() + 1);                   // increment player's score by 1
...
void Mole::handleTimeIsUp(GameLogic &game)
{
    ...
    if (game.getLives() > 0)
        ...
        game.setLives(0);                                 // ensure lives do not go negative
